...
impl Segment {
    ...
    pub fn collision(&self, particle: &Particle) -> Option<CollisionInfo> {
        ...
        let t = point_vec.dot(segment_vec) * segment_vec.dot(segment_vec).inv();
        ...
        let side = (particle.pos - self.p1).dot(self.normal);
        assert_dimension!(side, Length);

        // 4. Compute the collision velocity
        let collision_velocity = particle.vel.dot(collision_vector);

    
        if distance <= particle.radius && side >= Scalar::<Length>::zero() {
            Some(CollisionInfo {
                ke: (particle.mass * collision_velocity * collision_velocity).scale((0.5).dless()),
